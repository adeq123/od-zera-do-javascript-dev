...
/**
 * Function for figuring out what script a piece of text is using
 * @param {*} scripts 
 * @param {*} char 
 */
let characterScript = function (code) {
 ...
 * @param {*} text 
 */
let countBy = function (array, nameF) {
    let counts = [];
    array.forEach(element => {
        knownElementIndex = counts.findIndex(countsElement => countsElement.name == nameF(element));
        knownElementIndex >= 0 ? counts[knownElementIndex].count++ : counts.push({ name: nameF(element), count: 1 });
    }
    );
    return counts;
}
 ...
 * @return , the dominant writing direction
 */
function dominantDirection(text) {
    return countBy(Array.from(text), (char) => {
        let charScript = characterScript(char.codePointAt(0)); if (charScript != null) {
            return charScript.direction;
        } else {
            return 'none';
        }
    }).sort((directionOne, directionTwo) => directionTwo.count - directionOne.count)[0].name;
...
/*============== Testing Part ========================*/
//console.log(characterScript(121, SCRIPTS));
